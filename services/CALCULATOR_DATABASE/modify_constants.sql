UPDATE system.constants SET value = 'http://{{cp.pr.id}}.geoserver.{{cp.deployment.host}}:80/geoserver-calculator/wms?' WHERE key = 'GEOSERVER_CAPIBILITIES_ORIGINAL_URL';
